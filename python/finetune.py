...
class Geodesic_Loss(nn.Module):
    ...
    def forward(self, q1, q2):
        assert q1.shape == q2.shape, "Input shapes must match"
        assert q1.shape[-1] == 4, "Last dimension must be 4 (quaternion)"
...
def validation(
    ...
    criterion_geo_t = Geodesic_Loss().to(device)
    ...
    for i, (en_in_t, en_time_t, de_in_t, de_time_t, _) in enumerate(data_loader): 
        ...
        if args.use_feat == "all":
            ...
            forecast_traj_t = torch.cat([forecast_traj_right_t, forecast_traj_head_t, forecast_traj_left_t], dim=2)
            true_traj_t = torch.cat([true_traj_right_t, true_traj_head_t, true_traj_left_t], dim=2)
            mse_loss_t = criterion_mse_t(forecast_traj_t, true_traj_t)    # trajectory loss

            geo_loss_t = criterion_geo_t(forecast_quat_right_t, true_quat_right_t)  # quaternion loss
            geo_loss_t += criterion_geo_t(forecast_quat_head_t, true_quat_head_t)  # quaternion loss
            geo_loss_t += criterion_geo_t(forecast_quat_left_t, true_quat_left_t)  # quaternion loss
            ...
            exit()
        loss_t = 0.90 * mse_loss_t + 0.05 * geo_loss_t + 0.05 * bce_pressure_loss_t
...
if __name__ =="__main__":
    ...
    criterion_geo = Geodesic_Loss().to(device)
    ...
    for epoch in tqdm.tqdm(range(args.max_epoch)):
        ...
        for i, (en_in, en_time, de_in, de_time, _) in enumerate(V3_train_loader):
            ...
            if args.use_feat == "all":
                ...
                forecast_traj = torch.cat([forecast_traj_right, forecast_traj_head, forecast_traj_left], dim=2)
                true_traj = torch.cat([true_traj_right, true_traj_head, true_traj_left], dim=2)
                mse_loss = criterion_mse(forecast_traj, true_traj)    # trajectory loss

                geo_loss = criterion_geo(forecast_quat_right, true_quat_right)  # quaternion loss
                geo_loss += criterion_geo(forecast_quat_head, true_quat_head)  # quaternion loss
                geo_loss += criterion_geo(forecast_quat_left, true_quat_left)  # quaternion loss
            ...
            loss = 0.90 * mse_loss + 0.05 * geo_loss + 0.05 * bce_pressure_loss
