...
if __name__ == "__main__":
    ...
    x0 = torch.ones([bs, seq_len, num_features])
    x1 = torch.ones([bs, n_head, seq_len, num_features])
    x2 = torch.ones([bs, seq_len, d_model])


    attn = ScaledDotProductAttention(2)
    output, attn_score = attn(x1, x1, x1)
    print(output.size())
    print(attn_score.size())


    multi_attn = MultiHeadAttention()
    output, attn_score = multi_attn(x2, x2, x2)
    print(output.size())
    print(attn_score.size())


    posff = PoswiseFeedForwardNet()
    output = posff(output)
    print(output.size())

    encoder_layer = EncoderLayer()
    layer_output, layer_slf_attn = encoder_layer(x2)
    print(layer_output.size())
    print(layer_slf_attn.size())


    encoder = Encoder()
    enc_output, enc_attns = encoder(x0)
    print(enc_output.size())
    print(enc_attns[0].size())
